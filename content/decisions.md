...
| Connect VPCs in a single Region through a hub | **AWS Transit Gateway** — regional hub for thousands attachments, route table segmentation, Policy-Based Routing on source, port, and protocol | Simpler than Cloud WAN for single-Region and the only option supporting 5-tuple Policy-Based Routing; becomes complex to manage across Regions | [Connectivity Within AWS](connectivity/within-aws.md) |
...
| Connect AWS to another cloud provider privately | **AWS Interconnect** — managed direct cloud-to-cloud, MACsec with no keys to manage, 99.99% SLA per Interconnect, minutes to provision | Clouds and Region pairs keep expanding — check [Regional availability](https://docs.aws.amazon.com/interconnect/latest/userguide/region-availability.html). A missing pair isn't a blocker: land on a supported pair and extend inside each cloud; partner-based Direct Connect is for unsupported clouds. Hourly by bandwidth and distance tier, no per-GB, but a distant Cloud WAN Core Network Edge raises the tier | [Hybrid & Multi-Cloud](connectivity/hybrid-multicloud.md) |
...
| Verify which prefixes AWS and my on-premises routers are actually exchanging | **Direct Connect BGP route visibility** — accepted and advertised routes per VIF with AS path, community values, and install time, via console, `ListVirtualInterfaceRoutes`, or CLI | Point-in-time view scoped to the VIF; it does not show what transit gateway or Cloud WAN did with the route afterwards. Alarm on `VirtualInterfaceBgpPrefixes*` metrics to know when to look | [Hybrid & Multi-Cloud](connectivity/hybrid-multicloud.md) |
