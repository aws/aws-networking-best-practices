...
### Design Subnet Strategies Around Workload Communication Patterns

Poor subnet design can lead to unpredictable network costs and performance issues. Cross-Availability Zone (AZ) data transfer charges can add up when applications generate high-volume inter-tier communication. Additionally, subnet placement affects your ability to implement granular security controls and optimize routing paths. Before designing subnets, map your application's communication patterns. Create dedicated subnet ranges for each tier (web, application, and database) within each AZ. For example, when using a `10.0.0.0/16` VPC, allocate: `10.0.1.0/24` for the `web tier` in AZ-a, `10.0.11.0/24` for the `application tier` in AZ-a, `10.0.21.0/24` for the `database tier` in AZ-a. Repeat this pattern with incremental ranges for other AZs. This approach enables tier-specific routing policies and security group rules while providing clear network boundaries for troubleshooting. Reserve at least 25% of your VPC CIDR space for future growth. When planning IPv6 deployments, allocate your IPv6 CIDR block early and align your dual-stack strategy with your subnet boundaries to avoid complex renumbering later.
...
When using [Elastic Load Balancers](https://aws.amazon.com/elasticloadbalancing/), enable [cross-zone load balancing](https://docs.aws.amazon.com/elasticloadbalancing/latest/userguide/how-elastic-load-balancing-works.html) and implement health checks. Be sure to understand the associated cross-AZ data transfer costs that result from enabling this feature. Deploy sufficient capacity in each AZ and regularly test failover scenarios under realistic load conditions. Implement circuit breaker patterns in your application code to prevent retry storms when an AZ may become unavailable. Consider using [Application Recovery Controller (ARC) zonal shift](https://docs.aws.amazon.com/r53recovery/latest/dg/arc-zonal-shift.html), which allows you to redirect traffic from an impaired AZ to healthy AZs within the same AWS Region. Monitor the ELB `HealthyHostCount` [CloudWatch metric](https://docs.aws.amazon.com/elasticloadbalancing/latest/application/load-balancer-cloudwatch-metrics.html) per AZ and set up alerts when distribution varies by more than 20% from expected values. For applications requiring consistent performance across geographic regions, consider using [AWS Global Accelerator](https://aws.amazon.com/global-accelerator/).

### Optimize Cross-Region Connectivity for Cost and Performance

Cross-region data transfer can generate variable costs in multi-region deployments. Sub-optimal connectivity can cause data to follow inefficient network paths, resulting in increased latency and costs. Moreover, routing critical data over the internet introduces security and compliance risks that may violate regulatory requirements. Implement a hub-and-spoke connectivity model using either [AWS Transit Gateway](https://aws.amazon.com/transit-gateway/) or [AWS Cloud WAN](https://aws.amazon.com/cloud-wan/) for workloads that require multi-region communication. Establish a primary region as the hub and connect additional regions as spokes based on actual data flow requirements. For simple point-to-point connections between specific VPCs in different regions, use [VPC Peering](https://docs.aws.amazon.com/vpc/latest/peering/what-is-vpc-peering.html). Design routing tables to prioritize AWS backbone network paths over internet routing.

To reduce cross-region data transfer costs for AWS service API calls, deploy [AWS PrivateLink](https://aws.amazon.com/privatelink/) endpoints in each region. At the application layer, implement data compression and deduplication before initiating cross-region transfers. When appropriate, consider using [AWS DataSync](https://aws.amazon.com/datasync/) or [AWS Storage Gateway](https://aws.amazon.com/storagegateway/) for bulk data movement instead of application-level replication.
...
Deploy Local Zones specifically for workload components that require ultra-low latency for end users, including real-time gaming servers, financial trading applications, and content caching layers. While latency-sensitive components should be deployed in Local Zones, maintain core application logic and data stores in standard Availability Zones (AZs). Establish clear network paths between Local Zones and your main AZs, typically by using subnets within the same VPC. Since Local Zones have smaller resource pools compared to standard AZs, monitor their capacity constraints more carefully. Design your application architecture to handle Local Zone unavailability gracefully by implementing failback mechanisms to standard AZ resources.

### Design Data Residency and Compliance-Aware Network Boundaries

While compliance requirements often determine region selection, their implications for data residency obligations can be overlooked when designing network architecture. Network design can influence data flow patterns, and seemingly harmless configurations, such as CloudWatch log destinations or AWS Config snapshots, can unintentionally transfer data across regional boundaries. This complexity is further amplified in multi-region deployments where compliance requirements may vary between regions. To ensure compliance: Map all data flows within their architecture, including AWS service integrations, logging, monitoring, and backup destinations, Configure region-specific AWS service endpoints, Ensure services like CloudWatch, CloudTrail, and AWS Config store data in desired regions and Implement network-level controls using VPC endpoints and routing policies to prevent unintended data transfer to non-compliant regions
...
Unpredictable network performance can affect application reliability and user experience. Database replication, distributed caching, and real-time communication features become unreliable when network performance varies significantly. This unpredictability can make capacity planning difficult and can lead to over-provisioning or performance issues during peak usage periods. To address these challenges, establish baseline network performance metrics between all Availability Zones (AZs) in your deployment regions. Use enhanced networking features such as [Elastic Fabric Adapter (EFA)](https://aws.amazon.com/hpc/efa/), [Scalable Reliable Datagram (SRD)](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ena-express.html), and [placement groups](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/placement-groups.html) where appropriate for performance-critical workloads. Design your application architecture by implementing timeout and retry policies that account for network variability.

Monitor inter-AZ network latency using [Infrastructure Performance in AWS Network Manager](https://docs.aws.amazon.com/network-manager/latest/infrastructure-performance/what-is-nmip.html) and configure alerts for performance degradation. Use [AWS X-Ray](https://aws.amazon.com/xray/) to trace network performance impact on application requests. For applications requiring consistent low latency, consider using [cluster placement groups](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/placement-groups.html) within single AZs for critical components while maintaining cross-AZ deployment for resilience. Implement application-level metrics that correlate network performance with business outcomes.

### Operational Considerations

Effective day-2 operations for multi-AZ and multi-region architectures require monitoring and automation strategies. Implement monitoring of resource health, cross-AZ network performance, data transfer costs, and application availability metrics. Use [AWS X-Ray](https://aws.amazon.com/xray/) and [CloudWatch Log Insights](https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/AnalyzingLogData.html) to correlate network performance across AZs, and set up automated runbooks via [AWS Systems Manager](https://aws.amazon.com/systems-manager/) for common failure scenarios.

Monitor cross-AZ data transfer costs and identify unexpected traffic patterns that may indicate architectural inefficiencies. Use [AWS Cost Explorer](https://aws.amazon.com/aws-cost-management/aws-cost-explorer/) to analyze transfer patterns and optimize costs. For storage-intensive workloads, consider whether immediate cross-AZ replication is necessary or if eventual consistency models suffice.
