...
Planning your IP address strategy before creating subnets is important for long-term success. While a `/16` VPC with `/24` subnets might seem convenient initially, this approach can waste valuable IP space and create challenges when peering VPCs, connecting with on-premises networks, or scaling beyond a few hundred resources. Carefully selecting CIDR blocks upfront helps avoid IP address conflicts, which are complex and costly to resolve later. Making CIDR changes after deployment typically requires rebuilding infrastructure, coordinating maintenance windows, and potentially redesigning applications. A well-planned IP addressing scheme will support your infrastructure's growth and integration needs while maximizing address space efficiency.
...
Creating subnets in only 1-2 Availability Zones (AZs), especially in regions with `3+` AZs is a common pattern. AWS services like Application Load Balancers require subnets in at least two AZs. Having subnets in all AZs provides maximum flexibility for service deployment and reduce the blast radius due to any AZ impact. Create each subnet tier in every available AZ within your chosen region. Consider your actual needs balancing it against the cost: two AZs provide good protection against outages while keeping costs reasonable. If you do choose multiple AZs, create each subnet type in your selected zones and use automation to discover available AZs dynamically based on [AZ-IDs](https://docs.aws.amazon.com/ram/latest/userguide/working-with-az-ids.html) rather than hard-coding them.
