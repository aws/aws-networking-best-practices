...
[AWS Regions are geographic locations](https://aws.amazon.com/about-aws/global-infrastructure/regions_az/) that contain multiple data centers. Availability Zones (AZs) are isolated facilities within a Region, each equipped with independent power, cooling, and networking infrastructure. [AWS Local Zones](https://aws.amazon.com/about-aws/global-infrastructure/localzones/) are extensions of AWS Regions that are positioned closer to population centers, providing low-latency compute, storage, and database services. Together, these components form AWS's global infrastructure, enabling high availability, fault tolerance, and reduced latency for applications. AWS has multiple Availability Zones (typically three or more) in each Region. 


[AWS Regions are geographic locations](https://aws.amazon.com/about-aws/global-infrastructure/regions_az/) that contain multiple data centers. Availability Zones (AZs) are isolated facilities within a Region, each equipped with independent power, cooling, and networking infrastructure. Each Region contains multiple Availability Zones (typically three or more). AWS Local Zones are extensions of AWS Regions positioned closer to population centers, providing low-latency compute, storage, and database services. Together, these components form AWS's global infrastructure, enabling high availability, fault tolerance, and reduced latency for applications. [AWS Wavelength](https://aws.amazon.com/wavelength/) brings on-demand computing and storage services to communication service providers' networks, enabling customers to build and deploy applications that meet their data residency, low-latency, and resiliency requirements. AWS Wavelength offers operational consistency and familiar tools for automation.

### Design Around Workload Communication Patterns

Structure your subnets to match application tiers (web, app, db) within each Availability Zone. For example:

| **Tier** | **AZ-a** | **AZ-b** | **AZ-c** |
|----------|----------|----------|----------|
| **Web Tier** | 10.0.1.0/24 | 10.0.2.0/24 | 10.0.3.0/24 |
| **App Tier** | 10.0.11.0/24 | 10.0.12.0/24 | 10.0.13.0/24 |
| **DB Tier** | 10.0.21.0/24 | 10.0.22.0/24 | 10.0.23.0/24 |

This creates a pattern where anyone on your team can instantly identify the tier and AZ by looking at the subnet CIDR. Cross-Availability Zone (AZ) data transfer charges can add up when applications generate high-volume inter-tier communication. Additionally, subnet placement affects your ability to implement granular security controls and optimize routing paths. Before designing subnets, map your application's communication patterns. You can then create dedicated subnet ranges for each tier (web, application, and database) within each AZ. Repeat this pattern with incremental ranges for other AZs. This approach enables tier-specific routing policies and security group rules while providing clear network boundaries for troubleshooting. Plan for future growth by reserving an appropriate portion of your VPC CIDR space based on your anticipated expansion needs. When planning IPv6 deployments, allocate your IPv6 CIDR block early and align your dual-stack strategy with your subnet boundaries to avoid complex renumbering later. Note that the CIDR ranges shown are examples. Each organization must design its IP addressing scheme to meet its operational requirements.
...
[AWS Local Zones](https://aws.amazon.com/about-aws/global-infrastructure/localzones/) enable you to use select AWS services, such as compute and storage services, in locations closer to end users, providing low-latency access to locally running applications. Local Zones are connected to their parent Region through Amazon's redundant, high-bandwidth private network, ensuring that applications running in Local Zones have fast, secure, and seamless access to other AWS services. Note: 1) Not all AWS services are available in Local Zones. Refer to [AWS Local Zones](https://aws.amazon.com/about-aws/global-infrastructure/localzones/features/) for details. 2) AWS Local Zones are available in [select metropolitan areas](https://aws.amazon.com/about-aws/global-infrastructure/localzones) only.

Deploy Local Zones for workload components that require low latency for end users, for e.g., real-time gaming servers and financial trading applications. While latency-sensitive components should be deployed in Local Zones, maintain core application logic and data stores in standard Availability Zones (AZs). Establish clear network paths between Local Zones and your main AZs, typically by using subnets within the same VPC. Since Local Zones have smaller resource pools compared to standard AZs, monitor their capacity constraints more carefully. Design your application architecture to handle Local Zone unavailability gracefully by implementing fail back mechanisms to standard AZ resources.
