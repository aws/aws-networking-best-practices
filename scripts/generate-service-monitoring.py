...
def note_str(item: Dict[str, Any]):
    """
    Render a Notes bax
    """
    return f"    !!! {item['level']} \"{item['title']}\"\n        {item['note']}\n"

def gen_service(name:str, abbrev:str, metrics:List[Dict[str, Any]], metrics_notes:Optional[List[Dict[str, Any]]] = None, quotas:Optional[List[List[str]]] = None, quotas_url:str = '') -> str:
    ...
    if metrics_notes is not None:
        for item in metrics_notes:
            ret += note_str(item)
...
if __name__ == '__main__':
    ...
    text += gen_service('Application Load Balancer', 'ALB',
                        metrics=[{'type': 'metrics', 'ns': 'AWS/ApplicationELB', 'per': None, 'url': 'https://docs.aws.amazon.com/elasticloadbalancing/latest/application/load-balancer-cloudwatch-metrics.html', 'data':
                        ...
                        metrics_notes=[{'level': 'info', 'title': 'Remember', 'note': '`HTTPCode_*` metrics exist for ALB and Target. These metrics relate to the (ALB or Target) resource generating them. Consider monitoring both ALB and Target Group perspectives.'}],
                        quotas=None,
    ...
    text += gen_service('AWS Direct Connect', 'DX',
                        metrics=[{'type': 'text', 'v': 'Monitor your router’s incoming and outgoing BPS and PPS, along with number of routes advertised and received against quotas.'},
    ...
    text += gen_service('AWS Site-to-Site VPN', 'S2SVPN',
                        metrics=[{'type': 'text', 'v': 'Monitor your router’s incoming and outgoing BPS and PPS, and alarm on tunnel down or errors.'},
    ...
    text += gen_service('AWS Transit Gateway', 'TGW',
                        metrics=[{'type': 'metrics', 'ns': 'AWS/TransitGateway', 'per': 'attachment', 'url': 'https://docs.aws.amazon.com/vpc/latest/tgw/transit-gateway-cloudwatch-metrics.html', 'data':
    ...
    text += gen_service('Network Load Balancer', 'NLB',
                        metrics=[{'type': 'metrics', 'ns': 'AWS/NetworkELB', 'per': 'NLB', 'url': 'https://docs.aws.amazon.com/elasticloadbalancing/latest/network/load-balancer-cloudwatch-metrics.html', 'data':
    ...
    text += gen_service('Gateway Load Balancer', 'GWLB',
                        metrics=[{'type': 'metrics', 'ns': 'AWS/PrivateLinkEndpoints', 'per': None, 'url': 'https://docs.aws.amazon.com/vpc/latest/privatelink/privatelink-cloudwatch-metrics.html#endpoint-metrics-dimensions', 'data':
    ...
    text += gen_service('Route53 Endpoints, Resolver, and Resolver DNS Firewall', 'R53',
                        metrics=[{'type': 'metrics', 'ns': 'AWS/Route53Resolver', 'per': None, 'url': 'https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/monitoring-resolver-with-cloudwatch.html', 'data':
    ...
    text += gen_service('NAT Gateway', 'NAT',
                        metrics=[{'type': 'metrics', 'ns': 'AWS/NATGateway', 'per': None, 'url': 'https://docs.aws.amazon.com/vpc/latest/userguide/metrics-dimensions-nat-gateway.html', 'data':
    ...
    text += gen_service('Internet Gateway', 'IGW',
                        metrics=[{'type': 'text', 'v': 'Monitor [instances](#instances) for network traffic exceeded.'},
    ...
    text += gen_service('AWS PrivateLink', 'PL',
                        metrics=[{'type': 'text', 'v': 'Monitor the attached load balancer (see their entries)'},
    ...
    text += gen_service('Instances', 'Inst',
                        metrics=[{'type': 'metrics', 'ns': 'agent', 'per': 'network interface', 'url': 'https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/monitoring-network-performance-ena.html', 'data':
    ...
    text += gen_service('AWS Network Firewall', 'ANFW',
                        metrics=[{'type': 'metrics', 'ns': 'AWS/NetworkFirewall', 'per': None, 'url': 'https://docs.aws.amazon.com/network-firewall/latest/developerguide/monitoring-cloudwatch.html', 'data':
